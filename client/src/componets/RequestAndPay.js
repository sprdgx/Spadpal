...
function RequestAndPay({requests , getNameAndBalance}) {
  ...
  const { write: writeAddName, data: dataAddName } = useContractWrite(configAddName);

  const { isSuccess: isSuccessAddedName } = useWaitForTransaction({
    hash: dataAddName?.hash,
  })

  useEffect(()=>{
    if(isSuccess || isSuccessRequest || isSuccessAddedName){
      getNameAndBalance();
    }
  ...
  return (
    <>
      ...
      >
        ...
        <p>Add Your Name</p>
        <Input placeholder="Add Your Name" value={addname} onChange={(val)=>setAddName(val.target.value)}/>
      </Modal>
